...
URL = "https://www.iranintl.com/iran?page=1"
HEADERS = {
...
def inspect():
    ...
    # IranIntl typically uses <article> tags or specific divs
    ...
    for i, art in enumerate(articles[:5]):
        ...
        # Title
        # Usually h3 or h2 or a span inside an 'a' tag
        title_tag = art.find(["h3", "h4", "h2"])
        if not title_tag:
             # Fallback: look for span with specific classes if known, or just text in 'a'
             a_tag = art.find("a")
             if a_tag:
        ...
        # Summary
        # Often in a p tag or div with 'summary' class
        p_tag = art.find("p")
